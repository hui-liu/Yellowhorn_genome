...
# do
    # outdir=$stoutdir/${SAMPLE}
    # echo "htseq-count -f bam -r name -s no -a 10 -t exon -i gene_id -m union --nonunique=none $htoutdir/${SAMPLE}.bam $RefGene > $outdir/${SAMPLE}.count"
# done
...
#
# awk '{print $1"\t"$1}' xso_exp_sample | sed 's/_[0-9]*$//' | sed '1,1isample\tcondition'> sample_design.txt
